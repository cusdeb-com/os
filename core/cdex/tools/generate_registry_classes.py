...
def _unescape_registry(value: str) -> str:
    ...
    be decoded into actual control characters (tab, newline).
...
def _referenced_classes(sections: list[_RegistrySection]) -> set[str]:
    ...
    This is a preparatory pass that ``_select_records`` uses to decide which
    non-extension sections to keep.
    """
    classes: set[str] = set()

    for section in sections:
        # Match ".txt", not ".txt\ShellNew" or other subsections of it.
        if not (len(section.parts) == 1 and section.parts[0].startswith(".")):
            continue

        for record in section.records:
            # Only the unnamed default value (@) points to a class.
...
def _named_records(records: list[str], allowed_names: set[str]) -> list[str]:
    ...
    This is the record-level filter that ``_select_records`` applies once it
    has picked the allow-list for a section.

    Args:
        records: Raw value records from one registry section.
        allowed_names: Allowed case-folded value names.
    ...
    return [
        ...
        if (_value_name(record) or "").casefold() in allowed_names
    ]
...
def _select_records(
    ...
    This is the section-level filtering step: it picks the allow-list for
    each section, using ``_referenced_classes`` for the class whitelist and
    ``_named_records`` to apply the chosen allow-list.
...
def _normalize(record: str, provider_pattern: re.Pattern[str]) -> str:
    ...
    1. Remove ``hivexregedit`` string type annotations.
    2. Replace the possible source forms of an icon provider's path
       (``%SystemRoot%``, ``C:\\Windows``, ``C:\\ReactOS``) with the
       concrete path under ``C:\\reactos``, where the ReactOS payload is
       mounted in the CDEX Wine prefix. Concrete paths make icon lookup
       reliable after the generated fragment is imported into a Wine
       prefix.
    3. Re-encode the value's registry escaping, so it stays canonical
       regardless of how step 2 or the original export formatted it.
...
class ReactOSClassesValidator:
    ...
    def _check_roundtrip_sections(
        ...
        if missing:
            details = "\n  ".join(missing)
            _fail(f"generated registry lost data during hivex round-trip:\n  {details}")
